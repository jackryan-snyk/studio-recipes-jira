...
Look for the project name ("Impacted project:") in the Jira Ticket. Infer the repo name from the project name. Confirm this repo matches the current local directory. If they do not match, abort with error: "Local repository does not match the repository specified in the Jira ticket.
...
## Error Handling
 Situation | Action |
|-----------|--------|
| Auth error | If authentication to Atlassian or Snyk fails, retry once. If still failing STOP|
